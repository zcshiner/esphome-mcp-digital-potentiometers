...
bool mcp4xxx_digipot_spi_component::check_spi_CMDERR_(uint8_t *data) {
  ...
  ESP_LOGV(TAG, "SPI CMDERR check: 0b" BYTE_TO_BINARY_PATTERN ", CMDERR=%s",
           BYTE_TO_BINARY(data[0]), error_code ? "VALID" : "INVALID");
  if (error_code == 0) {
    ESP_LOGE(TAG, "CMDERR returned INVALID");
...
bool mcp4xxx_digipot_spi_component::write_mcp4xxx_register_(mcp4xxx_digipot_base::MCP4XXXAddresses address,
  ...
  // With SPI methods, set D9 to 1 for multiplexed SDI/SDO pin use
  ...
  if (   command == mcp4xxx_digipot_base::MCP4XXXCommands::INCREMENT
    ...
    this->disable();
    ESP_LOGV(TAG, "Wrote increment/decrement command: 0x%02X 0b" BYTE_TO_BINARY_PATTERN,
    ...
    this->disable();
    ESP_LOGV(TAG, "Wrote two bytes: 0x%02X.%02X 0b" BYTE_TO_BINARY_PATTERN "." BYTE_TO_BINARY_PATTERN,
